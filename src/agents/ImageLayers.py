...
def ImageEater(crop=(0,-1), size=84, interpolation=Image.NEAREST):
    ...
    return T.Compose([ #T.Lambda(show_shape('input')),
        T.Lambda(crop), #T.Lambda(show_shape('crop')),
        T.ToTensor(), # (H,W,C) -> tensor((C,H,W))
        #T.Lambda(show_shape('ToTensor')),
        T.Grayscale(), #T.Lambda(show_shape('Grayscale')),
        T.Resize(size, interpolation=interpolation), #T.Lambda(show_shape('Resize')),
        # squeeze channel to be stacked with other timeslices
        T.Lambda(torch.squeeze), #T.Lambda(show_shape('squeeze')),
    ])
